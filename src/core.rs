...
// password cache TTL (like CLI's in-memory pw cache, tapi kita pakai local TTL meta)
const TTL_MS: f64 = 60.0 * 60.0 * 1000.0; // 1 hour
...
fn is_blob_id(input: &str) -> bool {
    // mirror CLI heuristic:
    // - if ada '/' atau '\' anggap path (bukan blob id)
    if input.contains('/') || input.contains('\\') {
        return false;
    }
    // - panjang >20 dan hanya base64-ish chars
    if input.len() <= 20 {
        return false;
    }
    input.chars().all(|c| {
...
pub async fn send_proof(
    proof_input: String,        // from --proof-file / -p
    key_name: String,           // from --key-name / -k
    proving_system: String,     // from --proving-system / -s
    game: Option<String>,       // from --game / -g (optional)
    payload: Option<String>,    // from --payload / -d (optional JSON)
    elf_input: Option<String>,  // from --elf-file / -l (optional)
    password: String,
) -> Result<JsValue, JsValue> {
    log("🔍 [Step 1] Starting proof submission process...");

    if proof_input.is_empty() || key_name.is_empty() || proving_system.is_empty() {
        return Err(JsValue::from_str("Missing required fields: proof_file, key_name, or proving_system"));
    }

    // Load keystore, unlock private key with password
    let key_store = load_key_store()?;
    ...
    let secret_key = decrypt_secret(encrypted_secret, &password)
        .map_err(|_| JsValue::from_str("Failed to decrypt secret key. Please check your password."))?;

    let secret_key_array: [u8; 32] = secret_key.as_slice().try_into()
        .map_err(|_| JsValue::from_str("Invalid secret key length"))?;
    let signing_key = SigningKey::from_bytes(&secret_key_array);

    // Parse payload JSON (optional)
    ...
    // CLI ALWAYS sets "game": unwrap_or("8queens")
    let game_value = game.clone().unwrap_or_else(|| "8queens".to_string());
    ...
    if proof_is_blob {
        request_body["proof_blob_id"] = json!(proof_input);
    } else {
        // Browser TUI currently can't read local files from filesystem automatically.
        // We'll just treat non-blob input as "unsupported path".
        return Err(JsValue::from_str(
            "Local file paths are not supported in the web TUI yet. Please pass a Walrus blob ID for --proof-file/-p.",
        ));
    }

    // Handle optional ELF input
    let mut has_elf = false;
    if let Some(elf_raw) = elf_input.clone() {
        if !elf_raw.trim().is_empty() {
            has_elf = true;
            request_body["elf_filename"] = json!("program.elf");

            if is_blob_id(&elf_raw) {
                request_body["elf_blob_id"] = json!(elf_raw);
            } else {
                return Err(JsValue::from_str(
                    "Local ELF path is not supported in the web TUI yet. Please pass a Walrus blob ID for --elf-file/-l.",
                ));
            }
        }
    }

    // canonical_string per CLI spec:
    // - if ELF present:
    //   "proof:{...}\nelf:{...}\nproof_filename:{...}\nelf_filename:{...}\nproving_system:{...}"
    // - else:
    //   "proof:{...}\nproof_filename:{...}\nproving_system:{...}"

    let proof_value_for_sig = request_body
        .get("proof")
        .or_else(|| request_body.get("proof_blob_id"))
        .and_then(|v| v.as_str())
        .unwrap_or("");

    let canonical_string = if has_elf {
        let elf_value_for_sig = request_body
            .get("elf")
    ...
    // Sign canonical_string
    let signature_bytes = signing_key.sign(canonical_string.as_bytes());
    let signature_b64 = general_purpose::STANDARD.encode(signature_bytes.to_bytes());

    let public_key_b64 = key_pair.public_key_string.clone();

    // Serialize request body to string
    let body_str = serde_json::to_string(&request_body)
        .map_err(|e| JsValue::from_str(&format!("Failed to serialize request: {:?}", e)))?;

    // Call JS fetch proxy (acts like reqwest in CLI)
    ...
    // refresh TTL so password session stays alive (mirror CLI password cache idea)
    refresh_ttl();
...
fn load_key_store() -> Result<KeyStore, JsValue> {
    ...
    let json_str = storage.get_item("key_store")
