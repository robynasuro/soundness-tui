...
pub fn app() -> Html {
    ...
    let on_command_input = {
        ...
        Callback::from(move |e: InputEvent| {
            ...
            log(&format!("Raw input command: {:?}", input.value()));
    ...
    let on_password_input = {
        ...
        Callback::from(move |e: InputEvent| {
            let input: web_sys::HtmlInputElement = e.target_unchecked_into();
            log(&format!("Password input: {}", input.value()));
    ...
    let on_send_proof = {
        ...
        Callback::from(move |_| {
            ...
            let is_sending = is_sending.clone();
            wasm_bindgen_futures::spawn_local(async move {
                log(&format!("Sending proof with payload: {:?}", parsed.payload));
                match send_proof(
                    ...
                    Ok(result) => {
                        let response: HashMap<String, Value> = serde_wasm_bindgen::from_value(result).unwrap_or_default();
                        let mut output = String::new();
                        ...
                        output.push_str(&format!(
                            "📝 Message: {}\n",
                            response.get("message").and_then(|v| v.as_str()).unwrap_or("No message")
                        ));
                        output.push_str(&format!(
                            "🔧 Proving System: {}\n",
                            response.get("proving_system").and_then(|v| v.as_str()).unwrap_or("UNKNOWN")
                        ));
                        output.push_str(&format!(
                            "🔍 Proof Verification: {}\n",
                            response.get("proof_verification").and_then(|v| v.as_str()).unwrap_or("UNKNOWN")
                        ));
                        output.push_str(&format!(
                            "⛓️ Sui Transaction: {}\n",
                            response.get("sui_transaction").and_then(|v| v.as_str()).unwrap_or("UNKNOWN")
                        ));
                        output.push_str(&format!(
                            "🔗 Transaction Digest: {}\n",
                            response.get("transaction_digest").and_then(|v| v.as_str()).unwrap_or("")
                        ));
                        output.push_str(&format!(
                            "📦 Proof Blob ID: {}\n",
                            response.get("proof_blob_id").and_then(|v| v.as_str()).unwrap_or("")
                        ));
                        output.push_str(&format!(
                            "🔑 Program Blob ID: {}\n",
                            response.get("program_blob_id").and_then(|v| v.as_str()).unwrap_or("")
                        ));
                        output.push_str(&format!(
                            "🔍 Suiscan Link: {}\n",
                            response.get("suiscan_link").and_then(|v| v.as_str()).unwrap_or("")
                        ));
                        output.push_str("🌊 Walruscan Links:\n");
                        if let Some(walruscan) = response.get("walruscan_links").and_then(|v| v.as_object()) {
                            output.push_str(&format!(
                                "📦 Proof Data: {}\n",
                                walruscan.get("proof_data").and_then(|v| v.as_str()).unwrap_or("")
                            ));
                            output.push_str(&format!(
                                "🔑 VK: {}\n",
                                walruscan.get("vk").and_then(|v| v.as_str()).unwrap_or("")
                            ));
                        } else {
                            output.push_str("📦 Proof Data: \n");
                            output.push_str("🔑 VK: \n");
                        }
                        output.push_str("═══════════════════════════");
    ...
    let on_tab_select = {
        ...
        Callback::from(move |e: MouseEvent| {
            ...
            let tab = target.get_attribute("data-tab").unwrap_or_default();
            log(&format!("Switching to tab: {}", tab));
            selected_tab.set(tab);
        })
    };

    log("Rendering App component with tab-nav and app-footer");

    html! {
        <div class="app-container">
            <header class="app-header">
                <h1>{ "Soundness TUI" }</h1>
                <div class="version">{ format!("Version: {}", get_version()) }</div>
            </header>
            <nav class="tab-nav">
                <button
                    class={if *selected_tab == "generate" { "tab-btn active" } else { "tab-btn" }}
                    data-tab="generate"
                ...
                <button
                    class={if *selected_tab == "import" { "tab-btn active" } else { "tab-btn" }}
                    data-tab="import"
                    onclick={on_tab_select.clone()}
                >
                    { "Import Phrase" }
                ...
                <button
                    class={if *selected_tab == "list" { "tab-btn active" } else { "tab-btn" }}
                    data-tab="list"
                    onclick={on_tab_select.clone()}
                >
                    { "List Keys" }
                ...
                <button
                    class={if *selected_tab == "send" { "tab-btn active" } else { "tab-btn" }}
                    data-tab="send"
                    onclick={on_tab_select.clone()}
                >
                    { "Send Proof" }
            ...
            <footer class="app-footer">
                <p>{"Built with ❤️ by 0xcreamy"}</p>
                <div class="footer-links">
                    <a href="https://testnet.soundness.xyz" target="_blank">{"Testnet"}</a>
                    <span>{" • "}</span>
                    <a href="https://github.com/soundness" target="_blank">{"GitHub"}</a>
                </div>
            </footer>
        </div>
    }
...
pub fn start_app() -> Result<(), JsValue> {
    STARTED.call_once(|| {
        ...
        log("Soundness TUI application started successfully");
    });
    Ok(())
}